package com.yoke.yokepicturebackend.manager.websocket;
...
import com.yoke.yokepicturebackend.manager.websocket.disruptor.PictureEditEventProducer;
import com.yoke.yokepicturebackend.manager.websocket.model.PictureEditActionEnum;
...
//        String message = String.format("用户 %s 加入编辑", user.getUserName());
    ...
    protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
        ...
        User user = (User) session.getAttributes().get("user");
        Long pictureId = (Long) session.getAttributes().get("pictureId");
        // 根据消息类别处理消息(生产消息到 Disruptor 环形队列中)
...
//        String message = String.format("用户 %s 加入编辑", user.getUserName());
    ...
    /**
     * 广播消息给该图片所有连接的用户(支持排除掉某个 Session)
     *
     * @param pictureId
    ...
    public void broadcastToPicture(Long pictureId, PictureEditResponseMessage pictureEditResponseMessage, WebSocketSession excludeSession) throws IOException {
        ...
        if (CollUtil.isNotEmpty(sessionSet)) {
            ...
            // 配置序列化：将Long 类型序列化为 String 类型，解决精度丢失问题
